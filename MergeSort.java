...
public class MergeSort {
    ...
    */
    ...
    public MergeSort(ArrayList<Player> toSortList){
    ...
    private ArrayList<Player> sortList(ArrayList<Player> pl){
        if(pl.size() > 1){
            int half = pl.size()/2;
            ArrayList<Player> leftList = copyList(pl, 0, q);
            ArrayList<Player> rightList = copyList(pl, q, pl.size());
            leftList = sortList(leftList);
            rightList = sortList(rightList);
            return mergeList(pl, leftList, rightList);
        }
        return pl;
    }
    
    private ArrayList<Player> mergeList(ArrayList<Player> comb, ArrayList<Player> left, ArrayList<Player> right){
        ...
        while ( i < totElem) { 
            ...
            if ((li < l.size()) && (ri<r.size())) {
                if (left.get(li).getName().compareTo(right.get(ri).getName()) < 0) {
                    comb.set(i, new Player(left.get(li)));// = l[li];
                    i++;
                    li++;
                }
                else {
                    comb.set(i, new Player(right.get(ri)));//a[i] = r[ri];
                    i++;
                    ri++;
                }
            }
                ...
                if (ri >= r.size()) {
                    while (li < l.size()) {
                        //outPrint(".");
                        comb.set(i, new Player(left.get(li)));//a[i] = l[li];
                        li++;
                        i++;
                    }
                }
    ...
    public void sortListByID(){
        sortedList = idSort(sortedList);
    }
    
    private ArrayList<Player> idSort(ArrayList<Player> pl){
        if(pl.size() > 1){
            int half = pl.size()/2;
            ArrayList<Player> leftList = copyList(pl, 0, q);
            ArrayList<Player> rightList = copyList(pl, q, pl.size());
            leftList = idSort(leftList);
            rightList = idSort(rightList);
            return idSortMerge(pl, leftList, rightList);
        }
        return pl;
    }
    
    private ArrayList<Player> idSortMerge(ArrayList<Player> comb, ArrayList<Player> left, ArrayList<Player> right){
        int totElem = left.size() + right.size();
        //int[] a = new int[totElem];
        int i,li,ri;
        i = li = ri = 0;
        while ( i < totElem) { 
            //outPrint(".");
            if ((li < l.size()) && (ri<r.size())) {
                if (left.get(li).getID() < right.get(ri).getID()) {
                    comb.set(i, new Player(left.get(li)));// = l[li];
                    i++;
                    li++;
                }
    ...
    private ArrayList<Player> copyList(ArrayList<Player> list, int start, int end){
        ArrayList<Player> tempList = new ArrayList<Player>();
        int index = 0;
        for(int i = start; i < end; i++){
            tempList.add(index, new Player(list.get(i)));
        }
        return tempList;
    }
    /**
